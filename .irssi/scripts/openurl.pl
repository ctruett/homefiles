...
%urltypes = ( http => { regexp => qr#((?:https?://[^\s<>"]+|www\.[-a-z0-9.]+)[^\s.,;<">\):])#, cmd => '"$1"' },
              ftp  => { regexp => qr#((?:ftp://[^\s<>"]+|ftp\.[-a-z0-9.]+)[^\s.,;<">\):])#, cmd => '"$1"' },
...
foreach (keys %urltypes) {
    Irssi::settings_add_str($IRSSI{'name'}, 'openurl_app_'.$_, "firefox ".$urltypes{$_}->{cmd});
